...
async function fetchTopPositions(
  ...
  if (!apiKey) {
    throw new Error(
      "Missing NEXT_PUBLIC_1INCH_API_KEY environment variable. Set it in your .env to enable Aqua API.",
    );
  }
...
export function useTopPositions({
  ...
  const query = useQuery({
    ...
    queryFn: () => fetchTopPositions(chainIds, limit, sortBy),
    staleTime: 60_000,
    retry: 1,
    enabled: !!apiKey && chainIds.length > 0,
  });
