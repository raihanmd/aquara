...
var TAKER_TRAITS_DEFAULT = "0x00000000000000000000000000000000000000000041";
var RAW_BALANCES_ABI = [
  {
    name: "rawBalances",
    type: "function",
...
function hexToBytes4(hex) {
  ...
  for (let i2 = 0;i2 < out.length; i2++)
    out[i2] = parseInt(h.slice(i2 * 2, i2 * 2 + 2), 16);
  return out;
}
function bytesToHexLocal(b) {
  let s = "0x";
  for (let i2 = 0;i2 < b.length; i2++)
    s += b[i2].toString(16).padStart(2, "0");
  return s;
}
var onCronTrigger = (runtime2) => {
  const cfg = runtime2.config;
  const don = runtime2;
  const network295 = getNetwork({ chainFamily: "evm", chainSelectorName: cfg.chainSelectorName });
  if (!network295)
    throw new Error(`unknown chain selector: ${cfg.chainSelectorName}`);
  const evm = new ClientCapability(network295.chainSelector.selector);
  const http2 = new ClientCapability2;
  const fetchJson = (url) => {
    const response = http2.sendRequest(runtime2, { url, method: "GET", headers: {}, body: new Uint8Array(0) }).result();
    if (!ok(response))
  ...
  let makers = [];
  try {
    const body = fetchJson(`${cfg.apiBase}/users?chainId=8453`);
    const list = Array.isArray(body) ? body : [];
    makers = list.filter((m) => typeof m === "string");
    runtime2.log(`[watch] makers from api=${makers.length}`);
  } catch (e2) {
    runtime2.log(`[watch] users fetch failed, falling back to config: ${String(e2).slice(0, 120)}`);
  }
  ...
  if (makers.length === 0) {
    runtime2.log("[watch] no makers - nothing to do");
    return JSON.stringify({ summary: { makers: 0, positions: 0 }, reports });
  }
  for (const maker of makers) {
    runtime2.log(`[watch] maker=${maker}`);
    let strategies = [];
    try {
      const body = fetchJson(`${cfg.apiBase}/strategies?maker=${maker}&mode=aggressive`);
      const items = Array.isArray(body) ? body : body.items ?? [];
      strategies = items.filter((s) => s.strategyHash && s.strategyBytes);
    } catch (e2) {
      runtime2.log(`[watch] maker=${maker} strategies fetch failed: ${String(e2).slice(0, 160)}`);
      continue;
    }
    runtime2.log(`[watch] maker=${maker} aggressive=${strategies.length}`);
    for (const s of strategies) {
      const hash2 = s.strategyHash;
      const fromRow = [s.tokenA, s.tokenB].filter(Boolean);
      const fromApi = (s.tokens ?? []).slice(0, 2);
      const toks = fromRow.length >= 2 ? fromRow.map((a, k) => ({
        address: a,
        symbol: fromApi[k]?.symbol
      })) : fromApi;
      if (toks.length < 2) {
        runtime2.log(`[watch] ${hash2.slice(0, 10)} skip: need 2 tokens`);
        continue;
      }
      const [tA, tB] = toks;
      const pair = `${tA.symbol ?? tA.address.slice(0, 6)}/${tB.symbol ?? tB.address.slice(0, 6)}`;
      const evmCall = (to, dataHex) => {
        const reply = evm.callContract(don, {
          call: {
            from: hexToBytes4(ZERO_ADDRESS),
            to: hexToBytes4(to),
            data: hexToBytes4(dataHex)
          }
        }).result();
        return reply.data;
      };
      const readRaw = (token) => {
        try {
          const data = encodeFunctionData({
            ...
            args: [maker, cfg.app, hash2, token]
          });
      ...
      if (balA >= 0n && balB >= 0n && (balA > 0n || balB > 0n)) {
        try {
          const [makerAddr, traits, orderData] = decodeAbiParameters([{ type: "address" }, { type: "uint256" }, { type: "bytes" }], s.strategyBytes);
          const inFirst = balA > 0n;
          const tokenIn = inFirst ? tA.address : tB.address;
          const tokenOut = inFirst ? tB.address : tA.address;
          const data = encodeFunctionData({
            abi: QUOTE_ABI,
            functionName: "quote",
            args: [
          ...
          runtime2.log(`[watch] ${hash2.slice(0, 10)} quote reverted: ${String(e2).slice(0, 120)}`);
          quoteOk = false;
        }
      } else {
        runtime2.log(`[watch] ${hash2.slice(0, 10)} quote skipped: unreadable balances`);
      }
      let verdict;
      if (balA < 0n || balB < 0n)
        verdict = "unreadable";
        ...
        verdict = "thin-allowance";
      else
        verdict = "healthy";
      runtime2.log(`[watch] ${hash2.slice(0, 10)} pair=${pair} balA=${balA} balB=${balB} ` + `allowanceOk=${allowanceOk} quoteOk=${quoteOk} quotedOut=${quotedOut} verdict=${verdict}`);
      reports.push({
        strategyHash: hash2,
        pair,
        mode: s.mode,
  ...
  runtime2.log(`[watch] summary makers=${summary.makers} positions=${summary.positions} ` + `healthy=${summary.healthy} depleted=${summary.depleted} oor-suspect=${summary.oorSuspect} ` + `thin-allowance=${summary.thinAllowance} unreadable=${summary.unreadable}`);
